...
def writeMoves(analizando, current, num):
    ...
    print 'el analziando es: (' + str(analizando[0]) + ',' \
        + str(analizando[1]) + ')\n'
    print 'el movimiento a hacer en x es: ' + str(newMov[1]) \
        + ' en y es: ' + str(newMov[0]) + '\n'
    if analizando[0] > 0:
        print 'Der,' + str(abs(analizando[1])) + ',' + str(numeroMov) \
            + '\n'
...
for num in pin:
    if num == 1:
        ...
        print 'el nuevo current es: (' + str(uno[0]) + ',' \
            + str(uno[1]) + ')\n'
        current = uno
    elif num == 2:
        analizando = dos
        writeMoves(dos, current, num)
        print 'el nuevo current es: (' + str(dos[0]) + ',' \
            + str(dos[1]) + ')\n'
        current = dos
    elif num == 3:
        analizando = tres
        writeMoves(tres, current, num)
        print 'el nuevo current es: (' + str(tres[0]) + ',' \
            + str(tres[1]) + ')\n'
        current = tres
    elif num == 4:
        analizando = cuatro
